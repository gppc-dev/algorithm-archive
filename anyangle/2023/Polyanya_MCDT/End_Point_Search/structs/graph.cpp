...
//                cout << s << " ";
    ...
    vector<int> Graph::generate_DFS_ordering2() {
    ...
    vector<int> Graph::generate_DFS_ordering() {
        ...
        vector<int> DFS_ordering(0);
        // Mark all the vertices as not visited

        vector<bool> visited(number_of_vertices, false);
        stack<int>stack;
        Dijkstra dijkstra = Dijkstra(this);
        for(int source_node=0; source_node<number_of_vertices; ++source_node) {
            if(!visited[source_node]) {
                vector<int> spt = dijkstra.get_shortest_path_tree(source_node);
                ...
                while (!stack.empty()) {
                    ...
                    // to the stack.
                    for (int a = vertices[s]; a < vertices[s + 1]; ++a) {
                        if (!visited[out_vertices[a]] && spt[out_vertices[a]] == s) {
                            stack.push(out_vertices[a]);
                        }
                    }
                }
