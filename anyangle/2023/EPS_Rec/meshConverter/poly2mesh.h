...
// will have to make our own. Additionally, Fade2D has its own point index_data
// feature, but it doesn't have a triangle index_data feature.
// This map will satisfy both of these uses.
    ...
    void init(const string input_file) {
        ...
        traversable = fadeutils::create_traversable_zone(fin, dt);
    ...
    void save_mesh(const string output_file) {
        ...
        for (auto triangle: triangles) {
            ...
            const int vertex_index[] = {0, 1, 2};
            for (int i: vertex_index) {
                fout << " " << triangle->getCorner(i)->getCustomIndex();
            }
            ...
            for (int i: triangle_index) {
                ...
                if (cur_triangle == NULL ||
                    ...
                    fout << triangle_to_index[cur_triangle];
                }
            }
            fout << endl;
        }
    ...
    void convertPoly2Mesh(const std::string input_filename, const std::string output_filename) {
        init(input_filename);
        save_mesh(output_filename);
//    print_header();
//    print_vertices();
//    print_polys();
    }
