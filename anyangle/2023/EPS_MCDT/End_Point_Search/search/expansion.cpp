...
//    const PointLocation& ssst =  mesh.centroid_location_list[417];
//        const Point v1  =  mesh.mesh_vertices[544].p;
//        const Point v2  =  mesh.mesh_vertices[488].p;
//    Orientation  a  = get_orientation(root, node.left, node.right);
...
// vector. Returns number of successors generated.
                       ...
                       Successor* successors)
    {
        ...
        const int N = (int) V.size();
        const Point& root = (node.root == -1 ? start : mesh_vertices[node.root].p);
        const bool is_start = (root == start );
//    std::cout << "node:" << root <<std::endl;
//    std::cout << node.left <<std::endl;
//    std::cout << node.right <<std::endl;
        int out = 0;
...
#undef is_zero
            ...
            if ((root_eq_l || root_eq_r) &&(is_start)) {
            ...
            if(is_collinear(root, node.right, node.left)){
                return out;
            }
        }
