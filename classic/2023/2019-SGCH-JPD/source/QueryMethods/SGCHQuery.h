...
//  typedef Grid2DSGManager SGM;
  ...
  // One for CH type.
  SGCHQuery(Grid2D* grid, CHParam p, SGParam sg_p, std::string base_filename);
  ~SGCHQuery();
  ...
  std::string GetRSaveFilename() {
    return save_load_path_ + ".r";
  }
  std::string GetRSPCSaveFilename() {
    return save_load_path_ + ".rspc";
  }
  std::string GetSubSaveFilename() {
    return save_load_path_ + ".sub";
  }

  std::string GetCHSaveFilename() {
    return save_load_path_ + ".ch";
  }
