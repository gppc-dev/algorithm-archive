...
class Grid2D {
 public:
  ...
  Grid2D(std::vector<bool> cells, int width, int height, int padding = 1) {
    GenerateGrid(cells, width, height, padding);
  }
  void GenerateGrid(std::vector<bool> cells, int width, int height, int padding = 1);
  ...
  bool IsTraversable(const SgchXyLoc & l) const {
  ...
  // Convert between SgchXyLoc and linearized padded coordinates (nodeId in the
  // context of graphs).
  xyLin ToXYLin(int x, int y) const {
    return y_mult_ * y + x + padding_offset_;
  }
  xyLin ToXYLin(const SgchXyLoc & l) const {
    return y_mult_ * l.y + l.x + padding_offset_;
  }
  SgchXyLoc ToSgchXyLoc(xyLin c) const {
    return SgchXyLoc((c - padding_offset_) % y_mult_,
  ...
  void PrintGridInfo(std::ostream & out = std::cerr) const {
  ...
  void GetNeighbors(SgchXyLoc l, std::vector<SgchXyLoc> & neighbors,
