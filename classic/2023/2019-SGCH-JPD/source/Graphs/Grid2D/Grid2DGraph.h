...
class Grid2DGraph {
 public:
...
#ifndef SG_QUIET
    std::cerr << "Grid graph contains " << num_valid_nodes_ << " nodes and "
  ...
  nodeId ToNodeId(SgchXyLoc l) const {
    return grid_->ToXYLin(l);
  }
  SgchXyLoc ToSgchXyLoc(nodeId n) const {
    if (direction_extended_)
      return grid_->ToSgchXyLoc(grid_->ExtractXYLin(n));
    else
      return grid_->ToSgchXyLoc(n);
  }
  SgchXyLoc ToState(nodeId n) const {
    return ToSgchXyLoc(n);
  }
...
//*
  ...
  Distance GetEdgeLength(SgchXyLoc p1, SgchXyLoc p2) {
  ...
  std::string GetNodeName(nodeId n) const {
    ...
    SgchXyLoc l = grid_->ToSgchXyLoc(n);
    std::stringstream ss;
    ss << "(" << l.x << ", " << l.y << ")";
    return ss.str();
  }
...
#ifndef NO_HOG
  /*
  void DrawNode(const MapEnvironment *env, SgchXyLoc pos,
                double priority = 0) const {
    env->OpenGLPriorityDraw(SgchXyLoc(pos.x, pos.y), priority);
  }
  */
  void DrawNode(const MapEnvironment *env, nodeId n,
                double priority = 0) const {
    if (direction_extended_) {
      auto l = grid_->ToSgchXyLoc(grid_->ExtractXYLin(n));
      ...
      auto l = ToSgchXyLoc(n);
      grid_->DrawNode(env, l.x, l.y, priority);
    }
  }

  void DrawNode(const MapEnvironment *env, SgchXyLoc l, Direction2D d, double prioirty = 0) {
    grid_->DrawArrowNode(env, l.x, l.y, d, 0.5);
    /*
  ...
  void DrawEdge(const MapEnvironment *env, SgchXyLoc from, SgchXyLoc to) const {
    //*
    if (true)
    /*/
    if (direction_extended_)
...
//      env->GLDrawColoredLine(from.x, from.y, to.x, to.y);
  ...
  void DrawEdge(const MapEnvironment *env, nodeId n1, nodeId n2) const {
    SgchXyLoc l1 = ToSgchXyLoc(n1);
    SgchXyLoc l2 = ToSgchXyLoc(n2);
/*
    if (direction_extended_)
...
*/
//      DrawEdge(env, ToSgchXyLoc(n1), ToSgchXyLoc(n2));
  ...
  void DrawLevelNode(const MapEnvironment *env, nodeId n, int level,
                double priority = 0) const {
    // TODO
    env->OpenGLPriorityDrawLevel(ToSgchXyLoc(n), level, priority);
  }
  void DrawLevelEdge(const MapEnvironment *env, nodeId n1, int level1,
                     nodeId n2, int level2) const {
    // TODO
    env->GLDrawColoredLineLevel(ToSgchXyLoc(n1), level1, ToSgchXyLoc(n2), level2);
  }

  // HACKY EXTENSION TO DIRECTED
  // TODO
  nodeId ToDirectionExtendedNodeId(SgchXyLoc l, Direction2D d = dAll) const {
    return grid_->ToXYLin(l)*9 + d;
  }
  SgchXyLoc ToSgchXyLocWithoutDirection(nodeId n) const {
    return grid_->ToSgchXyLoc(n/9);
  }
 ...
 private:
  ...
  void ConstructGraph() {
    ...
    for (nodeId id = 0; id < GetNumAllNodes(); id++) {
      ...
      num_valid_nodes_++;
      std::vector<SgchXyLoc> neighbors;
      grid_->GetNeighbors(ToSgchXyLoc(id), neighbors, eight_neighbor_,
                          diagonal_rule_);
      num_directed_edges_ += neighbors.size();
