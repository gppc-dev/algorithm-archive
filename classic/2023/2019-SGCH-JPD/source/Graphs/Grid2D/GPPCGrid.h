...
class GPPCGrid {
 public:
  ...
  bool IsTraversable(SgchXyLoc l) const {return g_.IsTraversable(l);}
  ...
  // CONVERSIONS (between xyLin, xydLin, SgchXyLoc, direction2D)
  // To xyLin
  xyLin ToXYLin(int x, int y) const {return g_.ToXYLin(x,y);}
  xyLin ToXYLin(const SgchXyLoc & l) const { return g_.ToXYLin(l);}
  // From xyLin
  SgchXyLoc ToSgchXyLoc(xyLin c) const {return g_.ToSgchXyLoc(c);}
  // To xydLin
  xydLin ToXYDLin(xyLin c, Direction2D d) const
    {return (d + 1) * GetNumPaddedCells() + c;}
  xydLin ToXYDLin(SgchXyLoc l, Direction2D d) const
    {return ToXYDLin(ToXYLin(l), d);}
  // From xydLin
  xyLin ExtractXYLin(xydLin c) const {return c % GetNumPaddedCells();}
  SgchXyLoc ExtractSgchXyLoc(xydLin c) const {return ToSgchXyLoc(ExtractXYLin(c));}
  ...
  inline SgchXyLoc Move(SgchXyLoc from, Direction2D d, int num_steps = 1) const;
  inline bool CanMove(xyLin from, Direction2D d) const;
  inline bool CanMove(SgchXyLoc from, Direction2D d) const;
  inline void GetMoveDirectionDetails(SgchXyLoc from, SgchXyLoc to, Direction2D & c,
  ...
  void GetTautCanonicalSuccessorDirections(
      SgchXyLoc l, Direction2D d, std::vector<Direction2D> & directions) const {
  ...
  void TestDirections(SgchXyLoc l) const;
  ...
  void DrawArrowNode(const MapEnvironment *env, xydLin l,
    ...
    auto xy = ToSgchXyLoc(ExtractXYLin(l));
  ...
  void DrawDiagonalFirstEdge(const MapEnvironment *env, int x1, int y1, int x2, int y2) {
    Direction2D c, d;
    int md, mt;
    this->GetMoveDirectionDetails(SgchXyLoc(x1,y1), SgchXyLoc(x2,y2), c, d, mt, md);

    SgchXyLoc m = Move(SgchXyLoc(x1,y1), d, md);
  ...
  void DrawDiagonalFirstGoingRightEdge(const MapEnvironment *env, int x1,
    ...
    if (x1 > x2)
      m = Move(SgchXyLoc(x1,y1), c, mt-md);
...
inline SgchXyLoc GPPCGrid::Move(SgchXyLoc from, Direction2D d, int num_steps) const {
  return SgchXyLoc(from.x + delta_x_[d]*num_steps, from.y + delta_y_[d]*num_steps);
}
...
inline bool GPPCGrid::CanMove(SgchXyLoc from, Direction2D d) const {
  // If it is a cardinal move
...
inline void GPPCGrid::GetMoveDirectionDetails(SgchXyLoc from, SgchXyLoc to, Direction2D & c,
