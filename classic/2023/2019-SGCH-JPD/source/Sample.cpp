...
void InstallHandlers() {
  ...
  InstallKeyboardHandler(MyDisplayHandler, "Enter query",
                         "Enter two SgchXyLocs from the keyboard.", kAnyModifier,
                         'q');
...
void MyDisplayHandler(unsigned long windowID, tKeyboardModifier mod, char key) {
  switch (key) {
    ...
    case 'r':  // Random problem
    {
      ...
      start = SgchXyLoc(x1, y1);
      goal = SgchXyLoc(x2, y2);
  ...
  if (button == kRightButton) {
    switch (mType) {
      ...
      case kMouseUp: {
        ...
        start = SgchXyLoc(px1, py1);
        goal = SgchXyLoc(px2, py2);

        //(134, 107)
        //start = SgchXyLoc(134, 107);
        //goal = SgchXyLoc(134, 107);
