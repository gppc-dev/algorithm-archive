...
public:
		...
		pos_to_node_(width*height, -1)
	{
		for(int y=0; y<height_; ++y)
			for(int x=0; x<width_; ++x)
				if(field[x+y*width_]){
					node_to_pos_.push_back(SgchXyLoc(x,y));
	...
	SgchXyLoc operator()(int x)const{ return node_to_pos_[x]; }
	int operator()(SgchXyLoc p)const{ 
	...
	void reorder(const std::vector<int>&perm, const std::vector<int>&inv_perm){
		...
		std::vector<SgchXyLoc>new_node_to_pos_(node_count_);
...
private:
	...
	std::vector<SgchXyLoc>node_to_pos_;
};

inline
std::vector<WeightedArcGPPC> extract_graph(const Mapper&mapper){
	std::int16_t dx[] = {-1, 0, 1, -1, 1, -1, 0, 1};
	std::int16_t dy[] = {-1, -1, -1, 0, 0, 1, 1, 1};
	int dw[] = {1414, 1000, 1414, 1000, 1000, 1414, 1000, 1414}; 
	std::vector<WeightedArcGPPC>arc_list;
	for(int u=0; u<mapper.node_count(); ++u){
		auto u_pos = mapper(u);
		for(int d = 0; d<8; ++d){
			SgchXyLoc v_pos = {static_cast<std::int16_t>(u_pos.x + dx[d]), static_cast<std::int16_t>(u_pos.y + dy[d])};
			int v = mapper(v_pos);
			if(v != -1 && mapper(SgchXyLoc{u_pos.x, static_cast<std::int16_t>(u_pos.y+dy[d])}) != -1 &&  mapper(SgchXyLoc{static_cast<std::int16_t>(u_pos.x+dx[d]), u_pos.y}) != -1)
