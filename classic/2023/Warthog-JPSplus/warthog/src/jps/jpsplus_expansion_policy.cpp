...
warthog::jpsplus_expansion_policy::jpsplus_expansion_policy(warthog::gridmap* map)
    : jpsplus_expansion_policy(map, std::string())
{ }

warthog::jpsplus_expansion_policy::jpsplus_expansion_policy(warthog::gridmap* map, const std::string& preFilename)
    : expansion_policy(map->height() * map->width())
{
	map_ = map;
	jpl_ = new warthog::offline_jump_point_locator(map, preFilename);
}
