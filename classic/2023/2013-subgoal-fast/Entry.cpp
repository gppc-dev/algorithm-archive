...
void PreprocessMap(std::vector<bool> &bits, int width, int height, std::string &filename)
{
	// Create and save the subgoal graph
	entry.subgoalGraph = new SubgoalGraph(bits,width,height,filename.c_str(),MEMORY_LIMIT);
...
void *PrepareForSearch(std::vector<bool> &bits, int w, int h,  std::string &filename)
{
#ifndef NO_SAVE_LOAD
	// Read the subgoal graph
	entry.subgoalGraph = new SubgoalGraph(filename.c_str());
#endif
